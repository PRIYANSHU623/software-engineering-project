...
export default function Home() {
  ...
  const handleVerifyStudent = async (studentId, jobId) => {
    try {
      ...
      const response = await axios.post(`/api/admin/verify-student/${studentId}`, {
        jobId: jobId
      });
                  ...
                  <li key={student._id} className="py-4">
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="flex flex-col space-y-2">
                        {!student.verified && (
                          <button
                            onClick={() => handleVerifyStudent(student._id, selectedInterestedJob)}
                          ...
                          >
                            {verifyingStudent === student._id ? "Verifying..." : "Verify Student"}
                          </button>
                        )}
                        {student.verified && (
